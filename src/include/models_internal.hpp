...
struct InteractionStats {
	...
	string ToJSON() const {
		return "{\"sql_total\":" + to_string(sql_total) + ",\"sql_ok\":" + to_string(sql_ok) +
		       ",\"sql_error\":" + to_string(sql_error) + ",\"rounds\":" + to_string(rounds) +
...
static inline string StripSpecialTokens(const string &s) {
	static const char *const TOKENS[] = {"<|im_end|>", "<|im_start|>", "</s>",    "<s>",    "<|endoftext|>",
...
#define MODELS_LOG(SOURCE, EVENT, ...) DUCKDB_LOG(SOURCE, ModelsLogType, EVENT, StringUtil::Format(__VA_ARGS__))
