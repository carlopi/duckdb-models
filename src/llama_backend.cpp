...
bool LlamaState::Load(const string &model_path, int32_t n_ctx, int32_t n_gpu_layers,
	...
	if (!model) {
		shell_state.ShellPrintError(
		    StringUtil::Format("Error: failed to load llama model from '%s'\n", model_path));
		return false;
	}
...
static string ApplyChatTemplate(llama_model * /*model*/,
...
static std::vector<llama_token> Tokenize(const llama_vocab *vocab, const string &text,
	...
	int actual = llama_tokenize(vocab, text.c_str(), static_cast<int32_t>(text.size()),
	                            tokens.data(), n, add_special, /*parse_special=*/true);
	if (actual < 0) return {}; // still failed
...
static string TokenToPiece(const llama_vocab *vocab, llama_token token) {
	...
	if (n < 0) {
		...
		n = llama_token_to_piece(vocab, token, big.data(), static_cast<int32_t>(big.size()),
		                         /*lstrip=*/0, /*special=*/true);
		if (n > 0) return string(big.data(), static_cast<size_t>(n));
		return {};
	}
...
static string Generate(llama_context *ctx, const llama_vocab *vocab,
                       llama_sampler *sampler, BaseShellState &shell_state,
                       ClientContext &client_ctx,
                       uint32_t ctx_size, uint32_t n_prompt_tokens,
                       bool stream = true) {
	string response;
	...
	while (true) {
		...
		if (token < 0) break; // invalid / LLAMA_TOKEN_NULL
		...
		if (n_generated % 50 == 0) {
			...
			MODELS_LOG(client_ctx, "llama",
			           "generating... %u tokens in %.1fs (%.1f tok/s)",
			           n_generated, elapsed,
		...
		if (n_prompt_tokens + n_generated + 64 >= ctx_size) {
			MODELS_LOG(client_ctx, "llama", "context nearly full (%u + %u >= %u), stopping",
			           n_prompt_tokens, n_generated, ctx_size - 64);
			break;
		}
...
string RunLlamaLoop(ClientContext &context, BaseShellState &shell_state,
                    ...
                    const string &system_prompt_override, bool single_shot_sql,
	...
	const size_t max_result_chars = std::min(MAX_RESULT_CHARS,
	                                          static_cast<size_t>(ctx_size) / 6);
	MODELS_LOG(context, "llama", "max_result_chars=%llu",
	...
	roles.push_back("system");   contents.push_back(sys);
	roles.push_back("user");     contents.push_back(user_input);
	...
	while (sql_rounds <= max_worker_rounds) {
		...
		}
		{
			...
			MODELS_LOG(context, "llama", "prefill start: %llu tokens",
			           static_cast<unsigned long long>(tokens.size()));
		...
		MODELS_LOG(context, "llama", "response(%llu chars): %.200s",
		           static_cast<unsigned long long>(response.size()), response.c_str());
		...
		if (sql.empty() && !force_final) {
			...
			if (s != string::npos) {
				...
				string first_line = StripSpecialTokens(
				    nl != string::npos ? response.substr(s, nl - s) : response.substr(s));
		...
		if (!sql.empty() && IsSafeSQL(sql) && sql_rounds < MAX_SQL_ROUNDS) {
			auto request_final = [&](const string &reason, const string &msg) {
				...
				roles.push_back("assistant"); contents.push_back(response);
				roles.push_back("user");      contents.push_back(msg);
				force_final = true;
			};
			...
			if (stats) { stats->sql_total++; if (qresult.is_error) stats->sql_error++; else stats->sql_ok++; }
			...
			roles.push_back("assistant"); contents.push_back(response);
			roles.push_back("user");      contents.push_back(result_text);
			...
			string clean;
			{
				...
				if (s == string::npos) clean.clear();
				else clean = clean.substr(s);
			}
			if (clean.empty()) {
...
static string FormatSchemaAsCreateTable(const string &summary_text,
	...
	while (pos < summary_text.size()) {
		...
		string line = nl == string::npos ? summary_text.substr(pos)
		                                 : summary_text.substr(pos, nl - pos);
		pos = nl == string::npos ? summary_text.size() : nl + 1;

		if (header) { header = false; continue; }
		if (line.empty()) continue;

		size_t t1 = line.find('\t'); if (t1 == string::npos) continue;
		size_t t2 = line.find('\t', t1 + 1); if (t2 == string::npos) continue;
		...
		if (section != "schema" || tbl.empty() || col.empty()) continue;
		...
		if (enum_pos != string::npos) {
			...
			while (i < type_str.size()) {
				if (type_str[i] == '(')       depth++;
				else if (type_str[i] == ')') { if (--depth == 0) { i++; break; } }
				i++;
			}
			size_t n_vals = 1;
			for (size_t j = enum_pos; j < i; j++) if (type_str[j] == ',') n_vals++;
			if (n_vals > 5)
				type_str = type_str.substr(0, enum_pos) + "ENUM(" + to_string(n_vals) + " values)";
		}

		if (tables.find(tbl) == tables.end()) table_order.push_back(tbl);
	...
	if (result.empty()) return summary_text; // fallback
...
static string ExtractCommand(const string &text, const string &marker) {
	...
	while (pos < text.size()) {
		...
		if (at_line_start) {
			...
			while (val < text.size() && (text[val] == ' ' || text[val] == '\t')) val++;
			size_t end = text.find('\n', val);
			if (end == string::npos) end = text.size();
			// Trim trailing CR
			while (end > val && text[end - 1] == '\r') end--;
...
string RunCoordinatorLoop(ClientContext &context, BaseShellState &shell_state,
                          ...
                          const string &worker_style, InteractionStats *stats) {
	...
	std::vector<string> roles, contents;
	roles.push_back("system"); contents.push_back(COORDINATOR_SYSTEM_PROMPT);
	...
	string schema_ddl; // CREATE TABLE DDL, re-injected into each sqlcoder call
	{
		...
		if (!schema_qr.is_error && schema_qr.text != "(0 rows)\n") {
			...
			MODELS_LOG(context, "llama", "phase0: schema_ddl=%llu chars",
	...
	for (int round = 0; round < max_rounds; round++) {
		...
		roles.push_back("user"); contents.push_back(user_msg);
		...
		MODELS_LOG(context, "llama", "coordinator round=%d tokens=%llu remaining=%.0fs",
		           round, (unsigned long long)tokens.size(), remaining);

		// Prefill
		{
			auto *mem = llama_get_memory(coordinator.ctx);
			if (!mem) { shell_state.ShellPrintError("Error: llama_get_memory() null\n"); break; }
		...
		string response = Generate(coordinator.ctx, cvocab, sampler, shell_state, context,
		                           cctx_sz, static_cast<uint32_t>(tokens.size()), /*stream=*/false);
		...
		if (!final_ans.empty()) {
			...
			if (!backing_sql.empty()) {
				...
				for (size_t i = 0; i < backing_sql.size(); i++) {
					footer += StringUtil::Format("  [%llu] %s\n",
			...
			if (seen_failed_delegations.count(delegate_q)) {
				MODELS_LOG(context, "llama",
				           "coordinator: exact failed delegation repeated for '%s', blocking",
				           delegate_q.c_str());
			...
			string worker_ans = RunLlamaLoop(context, shell_state, worker, worker_input,
			                                 term_width, worker_prompt, is_sqlcoder, stats);
			...
			if (is_sqlcoder && worker_ans.substr(0, 4) == "SQL:") {
				...
				if (s != string::npos && !worker_failed) backing_sql.push_back(wql.substr(s));
			}
			...
			if (seen_direct_sql.count(direct_sql)) {
				MODELS_LOG(context, "llama",
				           "coordinator: repeated ASK_SQL blocked: %.120s", direct_sql.c_str());
			...
			if (IsSafeSQL(direct_sql)) {
				...
				if (stats) { stats->sql_total++; if (qr.is_error) stats->sql_error++; else stats->sql_ok++; }
				if (!qr.is_error) {
		...
		if (elapsed >= time_budget_seconds) {
			MODELS_LOG(context, "llama", "coordinator: time budget exhausted (%.1fs >= %.1fs)",
...
string RunCoordinatorLoopWithWorker(ClientContext &context, BaseShellState &shell_state,
	...
	for (int round = 0; round < max_rounds; round++) {
		...
		MODELS_LOG(context, "llama", "coordinator_with_worker round=%d tokens=%llu remaining=%.0fs",
		           round, (unsigned long long)tokens.size(), remaining);
		...
		if (!final_ans.empty()) {
			...
			if (seen_direct_sql.count(direct_sql)) {
				MODELS_LOG(context, "llama",
				           "coordinator_with_worker: repeated ASK_SQL blocked: %.120s", direct_sql.c_str());
				string describe_hint;
				if (direct_sql.find("duckdb_tables") != string::npos) {
					auto dr = ExecuteQuery(*context.db, "DESCRIBE duckdb_tables()");
					if (!dr.is_error) describe_hint = "duckdb_tables() columns:\n" + dr.text;
		...
		elapsed = std::chrono::duration<double>(std::chrono::steady_clock::now() - t_start).count();
		if (elapsed >= time_budget_seconds) {
			MODELS_LOG(context, "llama", "coordinator_with_worker: time budget exhausted (%.1fs >= %.1fs)",
			           elapsed, time_budget_seconds);
			roles.push_back("user");
			contents.push_back("Time budget exhausted. Write FINAL_ANSWER: now using what you have gathered.");
		}
